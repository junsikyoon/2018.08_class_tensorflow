...
# B3: weight initialization with truncated normal distr
def weight_variable(shape):
    initial = tf.truncated_normal(shape, stddev=0.1)
    return tf.Variable(initial)

# B4: bias initialization with 0.1
...
# 5x5 patch, accept 1 channel, and produce 8.
W_conv1 = weight_variable([5,5,1,8])
b_conv1 = bias_variable([8])
h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1)
h_pool1 = max_pool_2x2(h_conv1)

# B10: fully connected layer with 100 neurons: FC -> RELU
W_fc1 = weight_variable([7 * 7 * 8, 100])
b_fc1 = bias_variable([100])
h_pool2_flat = tf.reshape(h_pool1, [-1, 7 * 7 * 8])
