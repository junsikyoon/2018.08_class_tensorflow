...
seq_data = ['every', 'front', 'after', 'range', 'moist', 'crazy', 'allow', 'coast', 'train', 'quiet']
n_input = n_class = 26
n_stage = 4

test_data = ['abcd', 'efgh', 'tole', 'rang', 'allo', 'nokd']
...
outputs, states = tf.nn.dynamic_rnn(cell3, X, dtype=tf.float32)
...
for i, seq in enumerate(seq_data):
    print(seq[0:4], char_arr[guess[i]])

# B13: use test data
input_batch = test(test_data)
guess = sess.run(prediction, feed_dict={X: input_batch})
